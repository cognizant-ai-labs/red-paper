...
for dataset_index in range(len(dataset_name_list)):
    ...
    for run in range(RUNS):
        ...
        with tf.Session(graph=tf.Graph()):
            ...
            results_dir = "Results"
            result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),results_dir,'{}_exp_info_dropout_{}_run{}.pkl'.format(dataset_name, NN_info, run))
            with open(result_file_name, 'rb') as result_file:
                exp_info = pickle.load(result_file)

            test_NN_predictions = np.mean(exp_info["test_NN_predictions_list"], axis=0)
            train_NN_predictions = np.mean(exp_info["train_NN_predictions_list"], axis=0)
            ...
            rio_data["train_NN_predictions_softmax"] = softmax(train_NN_predictions, axis=1)
            rio_data["test_NN_predictions_softmax"] = softmax(test_NN_predictions, axis=1)
