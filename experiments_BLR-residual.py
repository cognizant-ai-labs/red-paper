...
for dataset_index in range(len(dataset_name_list)):
    ...
    for run in range(RUNS):
        ...
        # preprocess data
        if dataset_name in new_dataset_name_list:
            train_dataset = dataset.sample(frac=0.8,random_state=run+99)
        ...
        for i in range(len(rio_data["train_check"])):
            if rio_data["train_check"].reshape(-1)[i]:
                moderator_train_labels[i] = 1 - np.max(rio_data["train_NN_predictions_softmax"][i])
            else:
                moderator_train_labels[i] = -np.max(rio_data["train_NN_predictions_softmax"][i])
        ...
        for i in range(len(rio_data["test_check"])):
            if rio_data["test_check"].reshape(-1)[i]:
                moderator_test_labels[i] = 1 - np.max(rio_data["test_NN_predictions_softmax"][i])
            else:
                moderator_test_labels[i] = -np.max(rio_data["test_NN_predictions_softmax"][i])
        ...
        exp_info = {}
        exp_info["moderator_test_mean"] = mean_test + np.max(rio_data["test_NN_predictions_softmax"], axis=1)
        exp_info["moderator_train_mean"] = mean_train + np.max(rio_data["train_NN_predictions_softmax"], axis=1)
        exp_info["moderator_test_var"] = var_test
        exp_info["moderator_train_var"] = var_train
        result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_exp_info_BLR-res_{}_run{}.pkl'.format(dataset_name, NN_info, run))
