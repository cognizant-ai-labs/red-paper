...
import os
import time
from util import load_UCI121, dataset_read, RIO_variants_running, RIO_MRBF_multiple_running
import numpy as np
    ...
    for run in range(RUNS):
        ...
        with tf.Session(graph=tf.Graph()):
            ...
            for i in range(len(train_labels)):
                moderator_train_labels[i] = rio_data["train_NN_predictions_softmax"][i][rio_data["train_labels"].values.reshape(-1)[i]]
            moderator_test_labels = np.zeros(len(rio_data["test_check"]))
            for i in range(len(test_labels)):
                moderator_test_labels[i] = rio_data["test_NN_predictions_softmax"][i][rio_data["test_labels"].values.reshape(-1)[i]]

            moderator_train_data = normed_train_data.values
            moderator_test_data = normed_test_data.values
            model = build_regression_model(layer_width, len(normed_train_data.keys()))

            # The patience parameter is the amount of epochs to check for improvement
            early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)

            history = model.fit(moderator_train_data, moderator_train_labels, epochs=EPOCHS,
                                validation_split = 0.2, verbose=2, callbacks=[early_stop])

            moderator_test_NN_predictions = model.predict(moderator_test_data)
            moderator_train_NN_predictions = model.predict(moderator_train_data)
            exp_info = {}
            ...
            result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_exp_info_CondifNet_{}_run{}.pkl'.format(dataset_name, NN_info, run))
            with open(result_file_name, 'wb') as result_file:
                pickle.dump(exp_info, result_file)
            ...
            model = build_regression_model(layer_width, num_class)
            # The patience parameter is the amount of epochs to check for improvement
            early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)
            ...
            exp_info["moderator_test_NN_predictions"] = moderator_test_NN_predictions
            exp_info["moderator_train_NN_predictions"] = moderator_train_NN_predictions
            result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_exp_info_Introspection-Net_{}_run{}.pkl'.format(dataset_name, NN_info, run))
            with open(result_file_name, 'wb') as result_file:
                pickle.dump(exp_info, result_file)

            # TrustScore
            label_count = np.zeros(num_class)
            for i in range(len(rio_data["train_labels"].values.reshape(-1))):
                label_count[rio_data["train_labels"].values.reshape(-1)[i]] += 1
            if np.min(label_count) < 2:
                print("skip {} run {}".format(dataset_name, run))
                continue
            trust_model = trustscore.TrustScore()
            trust_model.fit(rio_data["normed_train_data"].values, rio_data["train_labels"].values.reshape(-1))
            trust_score_test = trust_model.get_score(rio_data["normed_test_data"].values, np.argmax(rio_data["test_NN_predictions"], axis=1))
            trust_score_train = trust_model.get_score(rio_data["normed_train_data"].values, np.argmax(rio_data["train_NN_predictions"], axis=1))
            exp_info = {}
            exp_info["trust_score_test"] = trust_score_test
            exp_info["trust_score_train"] = trust_score_train
            result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_exp_info_TrustScore_{}_run{}.pkl'.format(dataset_name, NN_info, run))
            with open(result_file_name, 'wb') as result_file:
                pickle.dump(exp_info, result_file)
