...
def build_BNN_classification_model(layer_width, num_class, input_dim, train_size):
    kl_divergence_function = (lambda q, p, _: dist.kl_divergence(q, p) / tf.cast(train_size, dtype=tf.float32))
    model = keras.Sequential([
        tfp.layers.DenseFlipout(layer_width, kernel_divergence_fn=kl_divergence_function, activation=tf.nn.relu, input_shape=[input_dim]),
        tfp.layers.DenseFlipout(layer_width, kernel_divergence_fn=kl_divergence_function, activation=tf.nn.relu),
        tfp.layers.DenseFlipout(num_class, kernel_divergence_fn=kl_divergence_function)
    ])
...
for dataset_index in range(len(dataset_name_list)):
    ...
    print("start exp for {}".format(dataset_name))
    ...
    for run in range(RUNS):
        ...
        with tf.Session(graph=tf.Graph()):
            ...
            train_size = int(minibatch_size*0.8)

            time_checkpoint1 = time.time()

            # training NN
            model = build_BNN_classification_model(np.dtype('int32').type(layer_width), np.dtype('int32').type(num_class), len(normed_train_data.keys()), train_size)
            ...
            result_file_name = os.path.join(os.path.dirname(os.path.abspath(__file__)),'Results','{}_exp_info_BNN_{}_run{}.pkl'.format(dataset_name, NN_info, run))
